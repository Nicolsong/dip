 ...
 */
 ...
 if(isset($_GET['key']))
 {
	...
	else
	{
		...
		$title = 'Ваш учебный аккаунт успешно активирован';
		$message = 'Поздравляю Вас, Ваш учебный аккаунт успешно активирован';
			
		sendMessageMail($email, CNE_MAIL_AUTOR, $title, $message);
			
		/*Перенаправляем пользователя на 
		нужную нам страницу*/
		header('Location:'. CNE_HOST .'?mode=reg&active=ok');
		exit;   
	}
 }
 ...
 if(isset($_POST['submit']))
 {
	...
	else
	{
		if(emailValid($_POST['email']) === false)
           $err[] = 'Не правильно введен E-мейл'."\n";
	}
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				$sql = 'INSERT INTO `'. CNE_DBPREFIX .'reg`
						VALUES(
								"",
								:email,
								:pass,
				...
				//Отправляем письмо для активации
				$url = CNE_HOST .'?mode=reg&key='. md5($salt);
				$title = 'Регистрация учебная';
				$message = 'Для активации Вашего акаунта пройдите по ссылке 
				<a href="'. $url .'">'. $url .'</a>';
				
				sendMessageMail($_POST['email'], CNE_MAIL_AUTOR, $title, $message);
				
				//Сбрасываем параметры (!Отличие от оригинала)
